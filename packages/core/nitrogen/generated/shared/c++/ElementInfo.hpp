...
namespace margelo::nitro::ennio {
  ...
  public:
    ElementInfo() = default;
    explicit ElementInfo(std::string testID, std::string type, std::optional<std::string> text, bool accessible, bool enabled, LayoutMetrics layout): testID(testID), type(type), text(text), accessible(accessible), enabled(enabled), layout(layout) {}
