...
export default function ModalStackScreen() {
  ...
  return (
    <View style={styles.container} testID="modal-stack-screen">
      ...
      <PressableScale testID="modal-open-first" style={styles.button} onPress={() => setFirst(true)}>
        <Text style={styles.buttonText}>Open modal 1</Text>
      </PressableScale>

      <Modal visible={first} transparent animationType="slide" onRequestClose={() => setFirst(false)}>
        <View style={styles.modalBg}>
          <View style={styles.modalCard} testID="modal-1-body">
            <Text style={styles.modalTitle}>Modal 1</Text>
            <PressableScale testID="modal-open-second" style={styles.button} onPress={() => setSecond(true)}>
              <Text style={styles.buttonText}>Open modal 2</Text>
            </PressableScale>
            <PressableScale testID="modal-close-first" style={styles.buttonAlt} onPress={() => setFirst(false)}>
              <Text style={styles.buttonText}>Close modal 1</Text>
            </PressableScale>
          </View>

          <Modal visible={second} transparent animationType="fade" onRequestClose={() => setSecond(false)}>
            <View style={styles.modalBg}>
              <View style={styles.modalCard} testID="modal-2-body">
                <Text style={styles.modalTitle}>Modal 2 (nested)</Text>
                <PressableScale testID="modal-close-second" style={styles.button} onPress={() => setSecond(false)}>
